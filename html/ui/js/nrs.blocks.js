...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.handleInitialBlocks = function(response) {
		...
		if (NRS.blocks.length < 10 && response.previousBlock) {
			...
			for (var i = 0; i < NRS.blocks.length; i++) {
				...
				rows += "<tr><td><a href='#' data-block='" + String(block.height).escapeHTML() + "' data-blockid='" + String(block.block).escapeHTML() + "' class='block'" + (block.numberOfTransactions > 0 ? " " : "") + ">" + String(block.height).escapeHTML() + "</a></td><td data-timestamp='" + String(block.timestamp).escapeHTML() + "'>" + NRS.formatTimestamp(block.timestamp) + "</td><td class=\"fixedWidthFont\" style=\"text-align:right;\">" + NRS.formatAmount(block.totalAmountNQT) + "</td><td class=\"fixedWidthFont\" style=\"text-align:right;\">"+ NRS.formatAmount(block.totalFeeNQT) + "</td><td>" + NRS.formatAmount(block.numberOfTransactions) + "</td></tr>";
			}
	...
	NRS.blocksPageLoaded = function(blocks) {
		...
		for (var i = 0; i < blocks.length; i++) {
			...
			rows += "<tr><td><a href='#' data-block='" + String(block.height).escapeHTML() + "' data-blockid='" + String(block.block).escapeHTML() + "' class='block'" + (block.numberOfTransactions > 0 ? " style='font-weight:bold'" : "") + ">" + String(block.height).escapeHTML() + "</a></td><td>" + NRS.formatTimestamp(block.timestamp) + "</td><td class=\"fixedWidthFont\" style=\"text-align:right;\">" + NRS.formatAmount(block.totalAmountNQT) + "</td><td class=\"fixedWidthFont\" style=\"text-align:right;\">" + NRS.formatAmount(block.totalFeeNQT) + "</td><td class=\"fixedWidthFont\" style=\"text-align:right;\">" + NRS.formatAmount(block.numberOfTransactions) + "</td><td>" + (block.generator != NRS.genesis ? "<a href='#' class=\"fixedWidthFont\" data-user='" + NRS.getAccountFormatted(block, "generator") + "' class='user_info'>" + NRS.getAccountTitle(block, "generator") + "</a>" : $.t("genesis")) + "</td><td>" + NRS.formatVolume(block.payloadLength) + "</td><td>" + Math.round(block.baseTarget / 153722867 * 100).pad(4) + " %</td></tr>";
		}
